		...
		columns : [ [ 
			...
			{field:'goodsname',title:'商品名称',width:100,editor:{type:'combobox',options:{
				...
				onSelect:function(record){
					...
					var priceEditor = $('#grid').datagrid('getEditor', {index:existEditIndex,field:'price'});
					// 设置编辑器上输入框的值
		...
		toolbar:[
			{
				...
				handler: function(){
					...
					// 如何获取所有行数
					var rows = $('#grid').datagrid('getRows');// 返回当前页的所有行。 
						...
						success : function(rtn) {
							$.messager.alert('提示', rtn.message, 'info',function(){
								if(rtn.success){
									...
									// 加载空的数据达到清空的效果
									$('#grid').datagrid('loadData',{total:0,rows:[],footer:[{num:0,money:0}]});
								}
							});
						}
					});
...
function getEditor(fieldName){
	return $('#grid').datagrid('getEditor', {index:existEditIndex,field:fieldName});
}
