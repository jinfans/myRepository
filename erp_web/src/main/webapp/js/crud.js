...
$(function(){
	...
	$('#btnSearch').bind('click',function(){
		...
		if (parseFloat($('#inprice1').val()) > parseFloat($('#inprice2').val())) {
			alert('请重新选择进货价查询范围！')
			return
		}
		
		if (parseFloat($('#outprice1').val()) > parseFloat($('#outprice2').val())) {
			alert('请重新选择销售价查询范围！')
			return
		}
			...
			buttons:[
				{
					...
					handler:function(){
						...
						if (filename == "") {
							alert("对不起，您还未选择导入文件，请选择文件后再导入，谢谢！")
							return;
						}
						if (filename.indexOf(".") <= 0) {
							alert("对不起，导入数据格式必须有后缀名哦，请您调整格式后重新导入，谢谢 ！")
							return;
						}
					    var fileext = filename.substring(filename.lastIndexOf("."), filename.length).toLowerCase()
						if (fileext != '.xls'){
							alert("对不起，导入数据格式必须是xls格式文件哦，请您调整格式后重新导入，谢谢 ！")
							return;
						}
						$.ajax({
